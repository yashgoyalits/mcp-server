...
export async function getLatestQuarterDetails(slug: string): Promise<CallToolResult> {
  ...
  if (!index) {
    return {
      content: [{ type: 'text', text: `No index found for slug "${slug}". Verify with get_available_stocks().` }],
      isError: true,
    };
  }
  ...
  if (!latestId) {
    return {
      content: [{ type: 'text', text: `Stock "${slug}" exists but has no quarterly data yet (latest.quarterly is null).` }],
      isError: true,
    };
  }
  ...
  if (!quarterly) {
    return {
      content: [{ type: 'text', text: `Index references "${latestId}" but file is missing in R2. Run /index/init to rebuild.` }],
      isError: true,
    };
  }
